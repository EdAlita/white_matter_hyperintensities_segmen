                ...
                if folder.is_dir():
                    for file in folder.glob('*.nii.gz'):
                        if 'lesion' in file.name:
                            ...
                            dest_file = task_path / dest_folder / (folder.name.split('_')[0] + '.nii.gz')
                            shutil.copy2(file, dest_file)
                        elif 'FLAIR' in file.name:
                            # For image files
                            suffix = '_0000.nii.gz'
                            dest_folder = 'imagesTs' if is_test else 'imagesTr'
                            dest_file = task_path / dest_folder / (folder.name.split('_')[0] + suffix)
                            shutil.copy2(file, dest_file)
...
if __name__ == "__main__":
     ...
     parser.add_argument("data_type", type=str,help="data type structure to create")
